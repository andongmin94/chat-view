...
**개발자용 연결 경로이지 배포된 다중 사용자 서비스가 아닙니다.** 브라우저 로그인·현재 실행 중 자동 갱신·선택적인 이 PC 연결 유지 코드는 구현되어 있습니다. 실계정 치지직 전체 검증, 운영용 다중 사용자 서비스, 송출 PC 연동과 검증된 투컴 영상 전달, 광고·HP·수익은 아직 완료되지 않았습니다. 독립 실행만으로 HDMI에서 HUD가 제외되지는 않습니다. 실제 방송에 쓰기 전 해당 영상 구성을 검증해야 합니다.

## 개발용 자체 채팅 연결

[platform/README.md](platform/README.md)에 따라 비공개 개발자 설정으로 로컬 연동 도구를 실행합니다. HUD의 **Ctrl+Alt+Shift+C** 연결창에서 서비스 origin을 입력하고 **로그인 / 연결**을 누르면 시스템 브라우저에서 치지직 로그인·채널 연결을 승인합니다. 표시 키를 복사할 필요는 없습니다. HTTPS가 기본이며 로컬 도구만 명시적 허용 후 `http://127.0.0.1:47831`을 사용합니다.

승인 후 읽기 권한은 현재 실행 중 자동 갱신됩니다. **이 PC에서 연결 유지**를 선택한 경우 갱신용 챗뷰 승인만 Windows 사용자 보호 저장소에 보관하여 다음 실행에서 자동 연결합니다. 선택하지 않으면 앱 종료 시 지속 승인 없이 다음 실행에 다시 로그인합니다. **연결 중지**는 현재 표시를 끝내고, **로그아웃**은 로컬 저장 승인과 서버측 해당 챗뷰 승인을 해제합니다. 치지직 개발자 비밀키·제공자 토큰·채팅 본문은 이 로컬 저장소에 넣지 않습니다.
...
개발/배포 검사의 구분은 [운영 원칙](docs/development-workflow.md)을 따릅니다. 진단 내보내기는 로컬 동작이며 채팅·로그인/갱신 자격 증명·URL을 제품 로그에 추가하지 않습니다. 공유 전 진단 파일을 확인합니다. 새 클라우드 계정 기능의 데이터 처리는 별도로 설계해야 합니다.
