...
import { createWindow, getMainWindow, createOverlayWindow, getOverlayWindow } from './window.js';
    ...
    try {
      ...
      setTimeout(async () => {
        try {
          ...
          setupIpcHandlers(getMainWindow, getOverlayWindow, createOverlayWindow); // IPC 핸들러 설정
        } catch (error) {
