    ...
    const std::wstring &status_tone)
{
    std::wstring output;
    output.reserve(
        96U + status_text.size() + status_tone.size());
    output.append(
  ...
  const mount = () => {
    ...
    if (!observer) {
      observer = new MutationObserver(() => {
        if (!pageStyle.isConnected || !host.isConnected) {
          mount();
        }
      });
...
if include_line not in source:
    if include_anchor not in source:
        raise RuntimeError("Could not locate webview-host include anchor")
    source = source.replace(include_anchor, include_anchor + include_line, 1)

bootstrap_pattern = re.compile(
    r'constexpr wchar_t kOverlayBootstrapScript\[\] = LR"JS\(.*?\n\)JS";',
    re.DOTALL,
)
source, count = bootstrap_pattern.subn(SECURE_BOOTSTRAP, source, count=1)
if count != 1:
    raise RuntimeError(f"Expected one overlay bootstrap, replaced {count}")

javascript_helper = re.compile(
    r'\nstd::wstring javascript_string\(const std::wstring &value\)\n\{.*?\n\}\n\n(?=HRESULT create_d3d_device)',
    re.DOTALL,
)
source, count = javascript_helper.subn("\n", source, count=1)
if count != 1:
    raise RuntimeError(f"Expected one JavaScript string helper, removed {count}")

web_message_setting = "settings->put_IsWebMessageEnabled(FALSE)"
if web_message_setting in source:
    source = source.replace(
        web_message_setting,
        "settings->put_IsWebMessageEnabled(TRUE)",
        1,
    )
elif "settings->put_IsWebMessageEnabled(TRUE)" not in source:
    raise RuntimeError("Could not locate WebView web-message setting")

apply_pattern = re.compile(
    r'void WebViewHost::apply_host_state\(\) noexcept\n\{.*?\n\}\n\n\} // namespace chatview',
    re.DOTALL,
)
apply_replacement = r'''void WebViewHost::apply_host_state() noexcept
{
    if (!ready_ || !webview_) {
        return;
    }
...
source, count = apply_pattern.subn(apply_replacement, source, count=1)
if count != 1:
    raise RuntimeError(f"Expected one apply_host_state implementation, replaced {count}")
...
hud_anchor = "add_executable(chat-view-hud WIN32\n    src/hud/main.cpp\n"
if "src/hud/host-state-message.cpp" not in cmake:
    if hud_anchor not in cmake:
        raise RuntimeError("Could not locate HUD target source list")
    cmake = cmake.replace(
        hud_anchor,
        hud_anchor
...
if "chat-view-host-state-message-test" not in cmake:
    if test_anchor not in cmake:
        raise RuntimeError("Could not locate BUILD_TESTING block")
    test_block = r'''if(BUILD_TESTING)
...
new = (
    "- sends bounded host state through a one-way JSON message into a closed Shadow DOM control layer;"
)
if old in architecture:
    architecture = architecture.replace(old, new, 1)
if new not in architecture:
    raise RuntimeError("Architecture did not document the isolated host-state channel")
