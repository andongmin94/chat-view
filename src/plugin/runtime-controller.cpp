...
void RuntimeController::stop() noexcept
{
    ...
    if (runtime_process_) {
        const DWORD wait_result = WaitForSingleObject(runtime_process_.get(), kRuntimeExitWaitMs);
        if (wait_result == WAIT_TIMEOUT) {
            blog(LOG_WARNING, "[ChatView OBS] HUD runtime did not exit within %lu ms",
...
{
    if (runtime_process_ && WaitForSingleObject(runtime_process_.get(), 0U) == WAIT_TIMEOUT) {
        return true;
    }

    if (runtime_process_) {
        const DWORD exit_code_result = WaitForSingleObject(runtime_process_.get(), 0U);
        blog(LOG_WARNING,
             "[ChatView OBS] HUD runtime exited unexpectedly (wait result %lu); restarting",
             static_cast<unsigned long>(exit_code_result));
...
{
    const std::wstring runtime_path = find_runtime_path();
    if (runtime_path.empty()) {
        return false;
    }

    const DWORD attributes = GetFileAttributesW(runtime_path.c_str());
    ...
    std::wstring command_line =
        L"\"" + runtime_path + L"\" --mapping \"" + mapping_name_ + L"\" --event \"" + event_name_ +
        L"\" --parent " + std::to_wstring(process_id);
    ...
    if (!CreateProcessW(
            ...
            &startup_info,
            &process_info)) {
        log_windows_error("CreateProcessW", GetLastError());
        return false;
    }
...
std::wstring RuntimeController::find_runtime_path() const
{
    ...
    const DWORD length =
        GetModuleFileNameW(module_handle, module_path.data(), static_cast<DWORD>(module_path.size()));
    if (length == 0U || length >= module_path.size()) {
        log_windows_error("GetModuleFileNameW", GetLastError());
        return {};
    }

    return (std::filesystem::path(module_path.data()).parent_path() / kRuntimeExecutableName).wstring();
}
